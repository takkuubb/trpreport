...
const SYSTEM_PROMPT = `あなたはAirbnb物件運営会社のデータアナリストです。Airbnb物件の運営データを分析し、オーナー様への月次報告書を作成します。
以下のスタンスを必ず守ってください：
- ビジネスライクで丁寧なトーンで書く
- データに基づく客観的な分析を行う
- 問題点に対する改善策は「運営側で検討・対応を進める」というスタンスで記述する
- オーナーに作業を丸投げしない
- 具体的な数値を引用して説明する`;

const DEFAULT_PROMPTS = {
  summary: `【運営からの月次報告】として、以下のデータを分析してください：\n- 月間予約額、稼働泊数、ADR（客室単価）、成約率の状況\n- 前年同月比での変化\n- 特に注目すべきポイントと、運営側での対応方針\n300文字程度で簡潔にまとめてください。`,
  funnel: `【ファネル分析からの報告】として、以下のデータを分析してください：\n- 閲覧→連絡→予約の各ステップの転換率\n- 離脱が大きいポイントの特定\n- 写真・タイトル・説明文の改善余地\n- 運営側で対応を進める具体的なアクション\n300文字程度で簡潔にまとめてください。`,
  pricing: `【価格戦略に関する報告】として、以下のデータを分析してください：\n- ADR（客室単価）の水準と変動\n- 予約リードタイムの傾向（直前予約が多いか等）\n- 価格設定の最適化に関する提案\n- 運営側で検討を進める料金調整案\n300文字程度で簡潔にまとめてください。`,
  trend: `【稼働状況に関する報告】として、以下のデータを分析してください：\n- 主要指標の前年比トレンド\n- 改善傾向にある指標と悪化傾向にある指標\n- 競合環境の変化の可能性\n- 運営側で進める付加価値向上策\n300文字程度で簡潔にまとめてください。`
};
